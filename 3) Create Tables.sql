USE EMAIL_DB
GO

-- Сваляме всички таблици
EXEC SP_DROP_TABLE 'BULK_OUTGOING_MESSAGES';
EXEC SP_DROP_TABLE 'BULK_INCOMING_MESSAGES'; 
EXEC SP_DROP_TABLE 'LANG_SUPPORT'; 
EXEC SP_DROP_TABLE 'FILES';            
EXEC SP_DROP_TABLE 'USER_MESSAGE_FOLDERS';
EXEC SP_DROP_TABLE 'USER_MESSAGES';     
EXEC SP_DROP_TABLE 'MESSAGE_RECIPIENTS';      
EXEC SP_DROP_TABLE 'FOLDERS';            
EXEC SP_DROP_TABLE 'MESSAGES';                  
EXEC SP_DROP_TABLE 'USERS';      
EXEC SP_DROP_TABLE 'COUNTRIES';
EXEC SP_DROP_TABLE 'LANGUAGES';
EXEC SP_DROP_TABLE 'USER_ROLES';

-- COUNTRIES
CREATE TABLE [LANGUAGES](
    [ID] INT IDENTITY(1,1) NOT NULL,
    [NAME] NVARCHAR(32) NOT NULL,
    [CODE] NVARCHAR(5) NOT NULL,
    CONSTRAINT PK_LANGUAGES_ID PRIMARY KEY(ID)
);
EXEC sp_addextendedproperty N'MS_Description', N'Table for languages', N'SCHEMA', N'dbo', N'TABLE', N'LANGUAGES';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Language ID', N'LANGUAGES', N'ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Name of the language', N'LANGUAGES', N'NAME';
EXEC SP_ADD_COLUMN_DESCRIPTION N'CODE of the language', N'LANGUAGES', N'CODE';
GO

-- COUNTRIES
CREATE TABLE [COUNTRIES](
    [ID] INT IDENTITY(0,1) NOT NULL,
    [NAME] NVARCHAR(32) NOT NULL,
    [PHONE_NUMBER_CODE] NVARCHAR(5) NOT NULL,
	[LANGUAGE_CODE] INT NOT NULL DEFAULT 1,
    CONSTRAINT PK_COUNTRY_ID PRIMARY KEY(ID)
);
EXEC sp_addextendedproperty N'MS_Description', N'Table for countries list', N'SCHEMA', N'dbo', N'TABLE', N'COUNTRIES';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Country ID', N'COUNTRIES', N'ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Name of the country', N'COUNTRIES', N'NAME';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Country dialing code prefix', N'COUNTRIES', N'PHONE_NUMBER_CODE';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Language for the country', N'COUNTRIES', N'LANGUAGE_CODE';
GO

-- USERS
CREATE TABLE [USER_ROLES](
    [ID] INT NOT NULL,
    [NAME] NVARCHAR(64) NOT NULL,
    CONSTRAINT PK_USER_ROLES_ID PRIMARY KEY(ID),
);
EXEC sp_addextendedproperty N'MS_Description', N'Table for user roles', N'SCHEMA', N'dbo', N'TABLE', N'USER_ROLES';
EXEC SP_ADD_COLUMN_DESCRIPTION N'ID of the role', N'USER_ROLES', N'ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Name of the role', N'USER_ROLES', N'NAME';
GO

-- USERS
CREATE TABLE [USERS](
    [ID] INT IDENTITY(1,1) NOT NULL,
    [NAME] NVARCHAR(64) NOT NULL,
    [EMAIL] NVARCHAR(128) NOT NULL,
    [PASSWORD] NVARCHAR(64) NOT NULL,
    [PHONE_NUMBER] VARCHAR(256) NOT NULL,
    [COUNTRY_ID] INT NOT NULL DEFAULT 0,
	[PHOTO] VARBINARY(MAX) NULL,
	[USER_ROLE_ID] INT NOT NULL DEFAULT 0,
	[PREFFERED_LANGUAGE_ID] INT NOT NULL DEFAULT 1,

    CONSTRAINT PK_USERS_ID PRIMARY KEY(ID),
    CONSTRAINT FK_USERS_COUNTRIES_COUNTRY_ID FOREIGN KEY (COUNTRY_ID) REFERENCES COUNTRIES(ID),
	CONSTRAINT FK_USER_ROLES_USER_ROLE_ID FOREIGN KEY (USER_ROLE_ID) REFERENCES USER_ROLES(ID),
	CONSTRAINT FK_USER_LANGUAGES_PREFFERED_LANGUAGE_ID FOREIGN KEY (PREFFERED_LANGUAGE_ID) REFERENCES LANGUAGES(ID)
);

CREATE INDEX IX_USERS_COUNTRY_ID 
ON USERS (COUNTRY_ID);

EXEC sp_addextendedproperty N'MS_Description', N'Table for system users', N'SCHEMA', N'dbo', N'TABLE', N'USERS';
EXEC SP_ADD_COLUMN_DESCRIPTION N'User ID', N'USERS', N'ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Full name of the user', N'USERS', N'NAME';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Email address of the user', N'USERS', N'EMAIL';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Password hash or secret', N'USERS', N'PASSWORD';
EXEC SP_ADD_COLUMN_DESCRIPTION N'User phone number', N'USERS', N'PHONE_NUMBER';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Foreign key to country', N'USERS', N'COUNTRY_ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Profile photo of the user (binary image)', N'USERS', N'PHOTO';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Role of the user', N'USERS', N'USER_ROLE_ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Preferred language of the user', N'USERS', N'PREFFERED_LANGUAGE_ID';
GO

-- FOLDERS
CREATE TABLE [FOLDERS] (
    [ID] INT IDENTITY(1,1) NOT NULL,
    [NAME] NVARCHAR(64) NOT NULL,
	[USER_ID] INT NOT NULL,
	[FOLDER_DIRECTION] SMALLINT NOT NULL,
	CONSTRAINT PK_FOLDERS_ID PRIMARY KEY(ID),
	CONSTRAINT FK_FOLDERS_USERS_USER_ID FOREIGN KEY ([USER_ID]) REFERENCES [USERS](ID)
);

CREATE INDEX IX_FOLDERS_USER_ID ON FOLDERS (USER_ID);
CREATE UNIQUE INDEX UX_FOLDERS_NAME_USER_ID_FOLDER_DIRECTION ON FOLDERS (NAME, USER_ID, FOLDER_DIRECTION);
GO

EXEC sp_addextendedproperty N'MS_Description', N'Table for user-defined folders', N'SCHEMA', N'dbo', N'TABLE', N'FOLDERS';
EXEC SP_ADD_COLUMN_DESCRIPTION N'ID', N'FOLDERS', N'ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Name of the folder', N'FOLDERS', N'NAME';
EXEC SP_ADD_COLUMN_DESCRIPTION N'ID of user who created the folder', N'FOLDERS', N'USER_ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Direction of messages inside the folder', N'FOLDERS', N'FOLDER_DIRECTION';
GO

-- MESSAGES
CREATE TABLE [MESSAGES] (
    [ID] INT IDENTITY(1,1) NOT NULL,
    [FROM_EMAIL] NVARCHAR(128) NOT NULL,
    [SUBJECT] NVARCHAR(128),
    [BODY] NVARCHAR(MAX),
    [DATE_OF_REGISTRATION] DATETIME,
    [DIRECTION] SMALLINT NOT NULL,
    [STATUS] SMALLINT NOT NULL,
	CONSTRAINT PK_MESSAGES_ID PRIMARY KEY(ID)
);

EXEC sp_addextendedproperty N'MS_Description', N'Table for storing message metadata and content', N'SCHEMA', N'dbo', N'TABLE', N'MESSAGES';
EXEC SP_ADD_COLUMN_DESCRIPTION N'ID', N'MESSAGES', N'ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Sender email address', N'MESSAGES', N'FROM_EMAIL';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Subject of the message', N'MESSAGES', N'SUBJECT';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Body/content of the message', N'MESSAGES', N'BODY';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Date/time the message was sent', N'MESSAGES', N'DATE_OF_REGISTRATION';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Message direction (incoming, outgoing, internal)', N'MESSAGES', N'DIRECTION';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Message status (draft, sent, etc.)', N'MESSAGES', N'STATUS';
GO

CREATE TABLE [FILES](
    [ID] INT IDENTITY(1,1) NOT NULL,
    [NAME] NVARCHAR(256) NOT NULL,
    [CONTENT] VARBINARY(MAX) NOT NULL,
    [MESSAGE_ID] INT NOT NULL,
    CONSTRAINT PK_FILES_ID PRIMARY KEY(ID),
    CONSTRAINT FK_FILES_MESSAGES_MESSAGE_ID FOREIGN KEY ([MESSAGE_ID]) REFERENCES [MESSAGES](ID)
);

CREATE INDEX IX_FILES_MESSAGE_ID ON FILES (MESSAGE_ID);
GO

EXEC sp_addextendedproperty N'MS_Description', N'Table for storing attachments/files linked to messages', N'SCHEMA', N'dbo', N'TABLE', N'FILES';
EXEC SP_ADD_COLUMN_DESCRIPTION N'File ID', N'FILES', N'ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'File name including extension', N'FILES', N'NAME';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Binary content of the file', N'FILES', N'CONTENT';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Message ID the file is attached to', N'FILES', N'MESSAGE_ID';
GO

-- MESSAGE_RECIPIENTS
CREATE TABLE [MESSAGE_RECIPIENTS] (
    [ID] INT IDENTITY(1,1) NOT NULL,
    [MESSAGE_ID] INT NOT NULL FOREIGN KEY REFERENCES MESSAGES(ID),
    [EMAIL] NVARCHAR(256) NOT NULL,
	[IS_OUR_USER] BIT NOT NULL DEFAULT 0,
	CONSTRAINT PK_MESSAGE_RECIPIENTS PRIMARY KEY (ID)
);

EXEC sp_addextendedproperty N'MS_Description', N'Table for message recipients', N'SCHEMA', N'dbo', N'TABLE', N'MESSAGE_RECIPIENTS';
EXEC SP_ADD_COLUMN_DESCRIPTION N'ID', N'MESSAGE_RECIPIENTS', N'ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Id of message', N'MESSAGE_RECIPIENTS', N'MESSAGE_ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Email address of recipient', N'MESSAGE_RECIPIENTS', N'EMAIL';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Is the recipient our user', N'MESSAGE_RECIPIENTS', N'IS_OUR_USER';
GO

CREATE UNIQUE INDEX UX_MESSAGE_RECIPIENTS_MESSAGE_ID_EMAIL ON MESSAGE_RECIPIENTS (MESSAGE_ID, EMAIL);
GO

-- USER_MESSAGES
CREATE TABLE [USER_MESSAGES] (
	[ID] INT IDENTITY(1,1) NOT NULL ,
    [USER_ID] INT NOT NULL,
    [MESSAGE_ID] INT NOT NULL FOREIGN KEY REFERENCES MESSAGES(ID),
    [IS_DELETED] BIT NOT NULL DEFAULT 0,
    [IS_READ] BIT NOT NULL DEFAULT 0,
    CONSTRAINT PK_USER_MESSAGES PRIMARY KEY (ID),
	CONSTRAINT FK_USER_MESSAGES_USERS FOREIGN KEY ([USER_ID]) REFERENCES [USERS](ID)
);

EXEC sp_addextendedproperty N'MS_Description', N'Table to store per-user message state', N'SCHEMA', N'dbo', N'TABLE', N'USER_MESSAGES';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Identifier', N'USER_MESSAGES', N'ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'User ID', N'USER_MESSAGES', N'USER_ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Message ID', N'USER_MESSAGES', N'MESSAGE_ID';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Whether the user has deleted the message', N'USER_MESSAGES', N'IS_DELETED';
EXEC SP_ADD_COLUMN_DESCRIPTION N'Whether the message has been read', N'USER_MESSAGES', N'IS_READ';
GO

CREATE INDEX IX_USER_MESSAGES_USER_ID ON USER_MESSAGES (USER_ID);
CREATE UNIQUE INDEX UX_USER_MESSAGES_USER_ID_MESSAGE_ID ON USER_MESSAGES (USER_ID, MESSAGE_ID);
GO

CREATE TABLE [BULK_INCOMING_MESSAGES](
	[ID] [INT] IDENTITY(1,1) NOT NULL,
	[RAW_DATA] [NVARCHAR](MAX) NOT NULL,
	[RECEIVED_DATE] [DATETIME] NULL,
	[INCOMING_MESSAGE_ID] [INT] NOT NULL,
	CONSTRAINT PK_BULK_INCOMING_MESSAGES PRIMARY KEY (ID),
	CONSTRAINT FK_RAW_INCOMING_MESSAGE_MESSAGE_ID FOREIGN KEY (INCOMING_MESSAGE_ID) REFERENCES MESSAGES(ID))

	EXEC sp_addextendedproperty N'MS_Description',N'Table for incoming message to be processed',N'SCHEMA',N'dbo',N'TABLE',N'BULK_INCOMING_MESSAGES'
	EXEC SP_ADD_COLUMN_DESCRIPTION  N'ID for incoming message to be processed',N'BULK_INCOMING_MESSAGES',N'ID';
	EXEC SP_ADD_COLUMN_DESCRIPTION  N'Raw data',N'BULK_INCOMING_MESSAGES',N'RAW_DATA';
	EXEC SP_ADD_COLUMN_DESCRIPTION  N'Time to be sent',N'BULK_INCOMING_MESSAGES',N'RECEIVED_DATE';
	EXEC SP_ADD_COLUMN_DESCRIPTION  N'refrence to message',N'BULK_INCOMING_MESSAGES',N'INCOMING_MESSAGE_ID';
GO

CREATE INDEX UX_BULK_INCOMING_MESSAGES_MESSAGE_ID ON BULK_INCOMING_MESSAGES (INCOMING_MESSAGE_ID);
GO

CREATE TABLE [BULK_OUTGOING_MESSAGES](
	[ID] [INT] IDENTITY(1,1) NOT NULL,
	[RAW_DATA] [NVARCHAR](MAX) NOT NULL,
	[SENT_DATE] [DATETIME] NULL,
	[OUTGOING_MESSAGE_ID] [INT] NOT NULL,
	CONSTRAINT PK_BULK_OUTGOING_MESSAGES PRIMARY KEY (ID),
	CONSTRAINT FK_RAW_OUTGOING_MESSAGE_MESSAGE_ID FOREIGN KEY (OUTGOING_MESSAGE_ID) REFERENCES MESSAGES(ID))

	EXEC sp_addextendedproperty N'MS_Description',N'Table for incoming message to be processed',N'SCHEMA',N'dbo',N'TABLE',N'BULK_OUTGOING_MESSAGES'
	EXEC SP_ADD_COLUMN_DESCRIPTION  N'ID for incoming message to be processed',N'BULK_OUTGOING_MESSAGES',N'ID';
	EXEC SP_ADD_COLUMN_DESCRIPTION  N'Raw data',N'BULK_OUTGOING_MESSAGES',N'RAW_DATA';
	EXEC SP_ADD_COLUMN_DESCRIPTION  N'Time to be sent',N'BULK_OUTGOING_MESSAGES',N'SENT_DATE';
	EXEC SP_ADD_COLUMN_DESCRIPTION  N'refrence to message',N'BULK_OUTGOING_MESSAGES',N'OUTGOING_MESSAGE_ID';
GO

CREATE INDEX UX_BULK_OUTGOING_MESSAGES_MESSAGE_ID ON BULK_OUTGOING_MESSAGES (OUTGOING_MESSAGE_ID);
GO

CREATE TABLE [USER_MESSAGE_FOLDERS] (
    [ID] INT IDENTITY(1,1) NOT NULL,
    [USER_MESSAGE_ID] INT NOT NULL,
    [FOLDER_ID] INT NOT NULL,
    CONSTRAINT PK_USER_MESSAGE_FOLDERS PRIMARY KEY (ID),
	CONSTRAINT FK_USER_MESSAGE_FOLDERS_USER_MESSAGE FOREIGN KEY (USER_MESSAGE_ID) REFERENCES USER_MESSAGES(ID) ON DELETE CASCADE,
	CONSTRAINT FK_USER_MESSAGE_FOLDERS_FOLDERS FOREIGN KEY (FOLDER_ID)REFERENCES FOLDERS(ID) ON DELETE CASCADE
);

CREATE INDEX IX_USER_MESSAGE_FOLDERS_FOLDER_ID ON USER_MESSAGE_FOLDERS (FOLDER_ID);
CREATE INDEX IX_USER_MESSAGE_FOLDERS_MESSAGE_ID ON USER_MESSAGE_FOLDERS (USER_MESSAGE_ID);
GO  

EXEC sp_addextendedproperty N'MS_Description',N'Table for incoming message to be processed',N'SCHEMA',N'dbo',N'TABLE',N'USER_MESSAGE_FOLDERS'
EXEC SP_ADD_COLUMN_DESCRIPTION  N'Identifier',N'USER_MESSAGE_FOLDERS',N'ID';
EXEC SP_ADD_COLUMN_DESCRIPTION  N'Time to be sent',N'USER_MESSAGE_FOLDERS',N'USER_MESSAGE_ID';
EXEC SP_ADD_COLUMN_DESCRIPTION  N'refrence to message',N'USER_MESSAGE_FOLDERS',N'FOLDER_ID';
GO

CREATE TABLE [LANG_SUPPORT] (
    [ID] INT IDENTITY(1,1) NOT NULL,
    [MESSAGE_NAME] VARCHAR(256) NOT NULL,
	[VALUE] NVARCHAR(512) NOT NULL,
	[CONTEXT] INT NOT NULL,
    [LANGUAGE_ID] INT NOT NULL,
    CONSTRAINT PK_LANG_SUPPORT_ID PRIMARY KEY (ID),
	CONSTRAINT FK_LANG_SUPPORT_LANGUAGE_ID FOREIGN KEY (LANGUAGE_ID) REFERENCES LANGUAGES(ID),
);

CREATE INDEX IX_LANG_SUPPORT_LAGUAGE_ID ON LANG_SUPPORT (LANGUAGE_ID);
GO

EXEC sp_addextendedproperty N'MS_Description',N'Table for message translations',N'SCHEMA',N'dbo',N'TABLE',N'LANG_SUPPORT'
EXEC SP_ADD_COLUMN_DESCRIPTION  N'Identifier',N'LANG_SUPPORT',N'ID';
EXEC SP_ADD_COLUMN_DESCRIPTION  N'Name of the message',N'LANG_SUPPORT',N'MESSAGE_NAME';
EXEC SP_ADD_COLUMN_DESCRIPTION  N'Message in the corresponding language',N'LANG_SUPPORT',N'VALUE';
EXEC SP_ADD_COLUMN_DESCRIPTION  N'Context specifting the use of the message',N'LANG_SUPPORT',N'CONTEXT';
EXEC SP_ADD_COLUMN_DESCRIPTION  N'Language of the message',N'LANG_SUPPORT',N'LANGUAGE_ID';
GO